...
class Database
{
    ...
    public function dbOpen($dbhost = '', $dbuser = '', $dbpass = '', $dbname = '', $dbtype = '', $dbprefix = '')
    {
        ...
        if ($environment == 'Production') {
            $dsn = getenv('MYSQL_DSN_PROD_BUG');
            $user = getenv('MYSQL_USER_PROD_BUG');
            $pwd = getenv('MYSQL_PASSWORD_PROD_BUG');
        } else {
            $dsn = getenv('MYSQL_DSN_DEV_BUG');
            $user = getenv('MYSQL_USER_DEV_BUG');
            $pwd = getenv('MYSQL_PASSWORD_DEV_BUG');
        }

        $this->dblink = NewADOConnection('pdo');
        $this->dblink->Connect($dsn, $user, $pwd);
        $this->dblink->dataProvider = 'mysql';
